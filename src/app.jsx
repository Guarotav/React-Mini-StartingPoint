...
const App = () => {
  ...
  const fillGrid = () => {
    setGrid((prevGrid) => {
      const newGrid = prevGrid.map((row) => row.map(() => selectedColor));
      return newGrid;
    });
  };
  ...
  const clearGrid = () => {
    setGrid((prevGrid) => {
      const newGrid = prevGrid.map((row) => row.map(() => "white"));
      return newGrid;
    });
  };
  ...
  return (
    <div className="app">
      ...
      <div className="controls">
        ...
        <button id="delete-row" onClick={deleteRow}>
          Delete Row
        </button>
        <button id="add-column" onClick={addColumn}>
          Add Column
        </button>
